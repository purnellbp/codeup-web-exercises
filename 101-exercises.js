...
function addToDone(message) {
    ...
    node.appendChild(textnode); // Appeand the text to <li>
...
// *Hint* Recommend finding and using a built-in JS operation to add to an array rather than recreating the arrafruits.
...
function isOdd(positiveOddNumber) {
    var result;

    if (positiveOddNumber % 2 !== 0) {
        result = true;
    } else if (positiveOddNumber % 2 === 0) {
        result = false;
    }
    return result;
}
...
function isEven(positiveEvenNumber) {
    var result;

    if (positiveEvenNumber % 2 === 0) {
        result = true;
    } else if (positiveEvenNumber % 2 !== 0) {
        result = false;
    }
    return result;
}
...
function timesTwoPlusThree(sum) {
    var a = sum * 2 + 3;
    return a;
}
...
function endsWithVowel(str) {
    ...
    let last = str.charAt(n - 1),
        first = str.charAt(n - n);
...
function firstToLast(str) {
    var y = str.splice(0, 1),
        z = str.splice(0),
        result = str.concat(z, y);
    return result;
}
...
function onlyOddNumbers(arr, invert = false) {
    //Change invert to true to filter out odd numbers and return even numbers
...
function onlyEvenNumbers(arr, invert = true) {
    //Change invert to true to filter out odd numbers and return even numbers
    for (let i = 0; i < arr.length; ++i) {
        const even = arr[i] % 2 === 0;
        if (even ^ invert) {
...
function hasEvens(arr, invert = false) {
    var isEven = false;
    for (let i = 0; i < arr.length; ++i) {
        const even = arr[i] % 2 === 0;
        if (even ^ invert) {
...
function hasOdds(arr, invert = true) {
    var isEven = false;
    for (let i = 0; i < arr.length; ++i) {
        const even = arr[i] % 2 === 0;
        if (even ^ invert) {
...
function shortestString(arr) {
    return arr.reduce(function(prevWord, currWord) {
...
function addOneToArray(num){
    let result = num.map(x => x + 1);
    return result;
}
...
function totalOfBookPrices(array){
    ...
    for (var i = 0; i < array.length; i++) {
        bookPrice += array[i].price;
    }

    //subTotal = bookPrice \ array.length;
    return bookPrice;
}
...
function getAverageBookPrice(array){
    ...
    bookPrice = bookPrice / array.length;
    return bookPrice;
}
...
function totalNumberOfItems(shoppingCart){
    ...
    for (var i = 0; i < shoppingCart.items.length; i++){
        result = result + shoppingCart.items[i].quantity;
    }
    return result;
}
...
function getAverageItemPrice(){
    var totalPrice = 0;
    for(var i = 0; i < shoppingCart.items.length; i++){
        totalPrice = totalPrice + shoppingCart.items[i].price;
    }
    return totalPrice = totalPrice / shoppingCart.items.length;
...
function getAverageSpentPerItem(){
    var totalPrice = 0;
    var cartLength = shoppingCart.items.length;
    var totalQuant = 0;
    var tmp = 0;
    for(var i = 0; i < cartLength; i++){
        tmp = tmp + (shoppingCart.items[i].quantity * shoppingCart.items[i].price);
        totalQuant = totalQuant + shoppingCart.items[i].quantity;


    }
    return tmp / totalQuant;
